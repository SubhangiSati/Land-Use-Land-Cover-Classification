...
print("RNN Accuracy(Using RMSProp Optimizer):", rnn_accuracy)
...
# Create a log directory for TensorBoard
log_dir = "logs/rnn_model_rmsprop/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")

# Define the TensorBoard callback
tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)

# Save the RNN model
model.save("rnn_model_rmsprop.h5")
